...
SELECT
  ...
  FROM (
    ...
    FROM (
      ...
      JOIN
        {{ref('tempDailyMarketHistorical')}} b
      ON
        ...
        AND a.dm_date
      JOIN
        {{ref('tempDailyMarketHistorical')}} c
      ON
        a.dm_s_symb = c.dm_s_symb
        AND a.fiftytwoweekhigh = c.dm_high
        AND c.dm_date BETWEEN DATE_SUB(a.dm_date,INTERVAL 12 MONTH)
        AND a.dm_date) dmh QUALIFY ROW_NUMBER() OVER (PARTITION BY dm_s_symb, dm_date ORDER BY sk_fiftytwoweeklowdate, sk_fiftytwoweekhighdate) = 1) fmh
  ...
  LEFT JOIN
    {{ref('tempSumpFiBasicEps')}} f
  ON
    f.sk_companyid = s.sk_companyid
    AND EXTRACT(QUARTER FROM fmh.dm_date) = EXTRACT(QUARTER FROM fi_qtr_start_date)
    AND EXTRACT(YEAR FROM fmh.dm_date) = EXTRACT(YEAR FROM fi_qtr_start_date)
