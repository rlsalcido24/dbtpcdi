...
FROM (
  ...
  FROM (
    SELECT
      ...
      t_dts,
      COALESCE( sk_createdateid, LAST_VALUE(sk_createdateid IGNORE NULLS) OVER (PARTITION BY tradeid ORDER BY t_dts ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) ) AS sk_createdateid,
      COALESCE( sk_createtimeid, LAST_VALUE(sk_createtimeid IGNORE NULLS) OVER (PARTITION BY tradeid ORDER BY t_dts ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) ) AS sk_createtimeid,
      COALESCE( sk_closedateid, LAST_VALUE(sk_closedateid IGNORE NULLS) OVER (PARTITION BY tradeid ORDER BY t_dts ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) ) AS sk_closedateid,
      COALESCE( sk_closetimeid, LAST_VALUE(sk_closetimeid IGNORE NULLS) OVER (PARTITION BY tradeid ORDER BY t_dts ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) ) AS sk_closetimeid,
      cashflag,
      t_st_id,
      t_tt_id,
      t_s_symb,
    ...
    FROM (
      ...
      ON
        FORMAT_TIMESTAMP('%H:%M:%S',t.t_dts) = dt.timevalue ) ) QUALIFY ROW_NUMBER() OVER (PARTITION BY tradeid ORDER BY t_dts DESC) = 1 ) trade
JOIN
