...
FROM (
    SELECT * FROM (
        ...
        FROM
            ...
            INNER JOIN {{ ref('tempDailyMarketHistorical') }} AS b
                ON
                    a.dm_s_symb = b.dm_s_symb
                    AND a.fiftytwoweeklow = b.dm_low
                    AND b.dm_date BETWEEN ADD_MONTHS(
                        a.dm_date, -12
                    ) AND a.dm_date
            INNER JOIN {{ ref('tempDailyMarketHistorical') }} AS c
                ON
                    a.dm_s_symb = c.dm_s_symb
                    AND a.fiftytwoweekhigh = c.dm_high
                    AND c.dm_date BETWEEN ADD_MONTHS(
                        a.dm_date, -12
                    ) AND a.dm_date
    ) AS dmh
...
--${dq_left_flg}
    ...
    LEFT JOIN {{ ref('tempSumpFiBasicEps') }} AS f
        ON
            ...
            AND QUARTER(fmh.dm_date) = QUARTER(f.fi_qtr_start_date)
            AND YEAR(fmh.dm_date) = YEAR(f.fi_qtr_start_date)
