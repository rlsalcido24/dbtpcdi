...
FROM (
    ...
    FROM (
        SELECT
            trade.tradeid,
            MIN(DATE(trade.t_dts))
                OVER (PARTITION BY trade.tradeid)
                AS createdate,
            trade.t_dts,
            COALESCE(
                trade.sk_createdateid,
            ...
            COALESCE(
                trade.sk_createtimeid,
                LAST_VALUE(trade.sk_createtimeid IGNORE NULLS)
                    OVER (
                        PARTITION BY trade.tradeid
                        ORDER BY
                            trade.t_dts
            ...
            COALESCE(
                trade.sk_closedateid,
                LAST_VALUE(trade.sk_closedateid IGNORE NULLS)
                    OVER (
                        PARTITION BY trade.tradeid
                        ORDER BY
                            trade.t_dts
            ...
            COALESCE(
                trade.sk_closetimeid,
                LAST_VALUE(trade.sk_closetimeid IGNORE NULLS)
                    OVER (
                        PARTITION BY trade.tradeid
                        ORDER BY
                            trade.t_dts
